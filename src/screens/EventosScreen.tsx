...
function tarjetaEvento(evento: Evento, pasado: boolean) {
  ...
  return (
    <View
      key={evento.id}
      style={[styles.eventoCard, pasado && styles.eventoCardPasado, evento.destacado && styles.eventoCardDestacado]}
    >
      {evento.destacado && !pasado && (
        <View style={[styles.topBorde, { backgroundColor: colorTipo }]} />
      )}
      <View style={styles.eventoFila}>
        {/* Fecha */}
        <View style={[styles.fechaBadge, { backgroundColor: pasado ? colors.border : colorTipo }, pasado && styles.fechaBadgePasado]}>
          <Text style={[styles.fechaDia, pasado && styles.textoPasado]}>
            {fechaObj.getDate()}
          </Text>
        ...
        <View style={styles.eventoInfo}>
          <View style={styles.eventoTituloFila}>
            <Text
              style={[styles.eventoTitulo, pasado && styles.textoPasado]}
              numberOfLines={2}
            >
              {evento.titulo}
            </Text>
          ...
          <View style={[styles.tipoBadge, { backgroundColor: colorTipo + '20' }]}>
            <Text style={[styles.tipoTexto, { color: colorTipo }]}>
              {etiquetasTipo[evento.tipo]}
            </Text>
          </View>
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.encabezado}>
        ...
        <View>
          ...
          <Text style={styles.encabezadoSub}>Parroquia Nuestra Señora del Carmen</Text>
        </View>
      </View>
      ...
      {proximos.length === 0 ? (
        ...
        <View style={styles.lista}>{proximos.map((e) => tarjetaEvento(e, false))}</View>
      )}
      ...
      {mostrarPasados && (
        <View style={styles.lista}>{pasados.map((e) => tarjetaEvento(e, true))}</View>
      )}
